...
```sql
-- 取引ログ（プレフィックス付き：複数アプリでSupabaseを共有するため）
CREATE TABLE kaso_trade__trade_logs (
...
-- ポジション管理
CREATE TABLE kaso_trade__positions (
    id SERIAL PRIMARY KEY,
...
- Vercel: Supabase の `kaso_trade__positions` テーブルに保存
